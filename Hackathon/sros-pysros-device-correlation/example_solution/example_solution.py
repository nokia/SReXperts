...
class Node:
    ...
    def _get_connection(self, lab_name):
        try:
            return connect(host=f"clab-{lab_name}-{self.node_name}", username="admin", password="admin", hostkey_verify=False)
        except ModelProcessingError as model_processing_error:
            raise SystemExit(model_processing_error)
        except Exception as error:
            raise SystemExit(error)
        
        
def read_clab_topology(lab_directory):
    with open("./" + lab_directory + "/topology-data.json", 'r') as f:
...
def identify_sros_nodes(clab_topo):
    ...
    for item in clab_topo['nodes']:
        if clab_topo['nodes'][item]['kind'] == "vr-sros":
...
def _bgp_enabled_devices(node_connections):
    ...
    for node in node_connections:
        try:
            data_output = node.connection.running.get('/nokia-state:state/router[router-name="Base"]/bgp/statistics/operational-state')
...
def _isis_enabled_devices(node_connections):
    ...
    for node in node_connections:
        try:
            data_output = node.connection.running.get('/nokia-state:state/router[router-name="Base"]/isis[isis-instance="0"]/oper-state')
            if data_output.data == 'up':
                nodes.append(node.node_name)
        except LookupError:
            pass 
...
def _ipv4_routes_in_rib(node_connections):
    ...
    for node in node_connections:
        try:
            nodes_ipv4.append((node.node_name, len(node.connection.running.get_list_keys('/nokia-state:state/router[router-name="Base"]/route-table/unicast/ipv4/route'))))
        except LookupError:
            pass
...
def _cpu_util_rankings(node_connections):
    ...
    for node in node_connections:
        try:
            cpu = node.connection.running.get('/nokia-state:state/system/cpu[sample-period="60"]/summary/usage/cpu-usage').data
            tmp[node.node_name] = float(cpu)
        except LookupError:
            pass
    sorted_cpu_nodes_high_low = list(reversed(sorted(tmp.items(), key=lambda x:x[1])))
...
def print_table(data):
    ...
    table = Table("Collated network information", columns=cols,
                  showCount="Number of metrics", summary=summary, width=width)
    table.print(rows)
