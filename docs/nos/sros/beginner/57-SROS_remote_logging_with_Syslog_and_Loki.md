...
With the proliferation of distributed computing systems, the logging systems had a solid boost both in terms of log collection techniques, aggregation, querying and visualization. Modern logging stacks and services emerged - Logstash, Loki, and dozens of cloud-provided logging solutions.
...
1. A syslog collector and preprocessor - [Syslog-ng](https://syslog-ng.github.io/admin-guide/040_Quick-start_guide/README)
2. A log ingestion agent for Loki - [Promtail](https://grafana.com/docs/loki/latest/send-data/promtail/)
3. A database for centralized log storage and querying - [Loki](https://grafana.com/docs/loki/latest/)
4. A visualization tool for querying and analyzing data - [Grafana](https://grafana.com/docs/grafana/latest/)

The following diagram illustrates the logging stack:

-{{image(url='./../../../../../images/57_SROS_Syslog/stack.png', title='Fig. 2 - Logging stack')}}-
...
It is also worth noting that syslog messages are sent to Syslog-ng on UDP port 5514.
...
/// note
You may use Edgeshark (WebUI or from VSCode) or run TCPDump/TShark on your group's hackathon instance. For more details about the capture options refer to tools guide <a href="../../../../tools/tools-packet-capture" target="_blank" rel="noopener noreferrer"> Containerlab capture traffic </a>.
///

On your hackathon instance, start a packet capture in the `clab-srexperts-syslog` network namespace and trigger logs on the router. You may filter the packets by port `5514`.
...
- log in to or out of the router
...
For now, use one of the other options. For example, run the following `gnmic` commands on your hackathon instance to generate several logs.
...
On your hackathon instance, run the following TShark command in the `clab-srexperts-syslog` network namespace (or use TCPDump instead):
...
The Fig. 3 below shows the wireshark output using Edgeshark for syslog capture.

-{{image(url='./../../../../../images/57_SROS_Syslog/sros_syslog.jpg', title='Fig. 3 - SR OS syslog capture')}}-
...
On your hackathon instance, run the following command to view the Syslog-ng configuration file:
/// tab | syslog-ng.conf
``` bash
cat ~/SReXperts/clab/configs/syslog/syslog-ng.conf
```
///
...
* receives the raw syslog messages from SR OS on UDP port 5514
...
* forwards the enriched logs to Promtail on UDP port 1514 and to a local log
...
On your hackathon instance, start a new packet capture in the `clab-srexperts-syslog` network namespace. Apply a filter to capture both the logs received from SR OS nodes and those sent to Promtail. Trigger log events on the router as in the previous task.
...
-{{image(url='./../../../../../images/57_SROS_Syslog/syslog_to_promtail.jpg', title='Fig. 4 - Syslog to Promtail capture')}}-
...
Promtail will receive the syslog messages from Syslog-ng and generate additional labels associated with the log entry. The log processing/transformation is a very important process, it allows you to enrich, parse and tune the log messages before they are sent to the log collector.

Inspect the Promtail configuration file and try to interpret each section.

On your hackathon instance, run the following command to view the Promtail configuration file:
...
Promtail uses the Loki Push API at `/loki/api/v1/push`, with payload data serialized using Protocol Buffers.
```yaml
clients:
  - url: http://loki:3100/loki/api/v1/push
```
...
-{{image(url='./../../../../../images/57_SROS_Syslog/promtail.JPG', title='Fig. 5 - Promtail')}}-
...
Loki is the central log aggregation server.
Inspect the Loki configuration file and try to interpret each section.

On your hackathon instance, run the following command to view the Loki configuration file:
/// tab | loki-config.yml
``` bash
cat ~/SReXperts/clab/configs/loki/loki-config.yml
```
///
/// tab | Output
The most relevant setting is HTTP port 3100, which Promtail and Grafana use to communicate with Loki.
Loki stores logs pushed by Promtail and exposes an API that Grafana uses to query them.
``` bash
$ cat ~/SReXperts/clab/configs/loki/loki-config.yml
# Copyright 2023 Nokia
# Licensed under the BSD 3-Clause License.
...
Open the [Grafana](https://grafana.com) UI on your laptop at the following address, replacing `<group-id>` with your group ID:
```
http://<group-id>.srexperts.net:3000
```


Log in to the Grafana dashboard using the credentials provided to you.
...
-{{image(url='./../../../../../images/57_SROS_Syslog/loki-nav-sros.gif', title='Fig. 6 - Loki')}}-
...
Grafana acts as the client, initiating queries and requesting visualization data. Loki acts as the server, processing the queries and returning the log data.
Inspect the Grafana configuration file and try to interpret each section.

On your hackathon instance, run the following command to view the Grafana configuration file:
/// tab | datasource.yml
``` bash
cat ~/SReXperts/clab/configs/grafana/datasource.yml
```
///
/// tab | Output
The relevant configuration for this activity is the Loki data source. Prometheus is used for the streaming telemetry activities.
``` bash
$ cat ~/SReXperts/clab/configs/grafana/datasource.yml
# Copyright 2023 Nokia
# Licensed under the BSD 3-Clause License.
...
Open the Grafana UI on your laptop at the following address, replacing `<group-id>` with your group ID:
```
http://<group-id>.srexperts.net:3000
```
...
-{{image(url='./../../../../../images/57_SROS_Syslog/grafana.JPG', title='Fig. 7 - Grafana')}}-
...
The Fig. 8 below compare a real event with the corresponding test event:
-{{image(url='./../../../../../images/57_SROS_Syslog/test-event_vs_real-event.png', title='Fig. 8 - Real event vs. test event')}}-
...
-{{image(url='./../../../../../images/57_SROS_Syslog/fan_failure.jpg', title='Fig. 9 - Fan failure test logs')}}-
