...
class Plugin(CliPlugin):
    def load(self, cli, **_kwargs):
        syntax = Syntax(
            "bgp-by",
            help="Displays brief but useful data related to BGP peers. \
        ...
        syntax.add_named_argument(
            "peer-as", default="*", value_checker=CheckASN(), help="peer as number"
        )
        syntax.add_named_argument(
            "peer-address",
            default="*",
        ...
        syntax.add_named_argument(
            ...
            default="*",
            suggestions=KeyCompleter(
                path="/network-instance[name=*]/protocols/bgp/group[group-name=*]"
            ),
            help="bgp group name",
        )
    ...
    def _my_schema(self):
        ...
        netinst = root.add_child(
            "network-instance", key="name", fields=["name", "local-as"]
        )
    ...
    def _fetch_state(self, state, arguments):
        local_as_path = build_path(
            "/network-instance[name={name}]/protocols/bgp/autonomous-system",
            name=arguments.get("network-instance"),
        )
        peer_path = build_path(
            "/network-instance[name={name}]/protocols/bgp/neighbor[peer-address={peer_address}]",
            name=arguments.get("network-instance"),
            peer_address=arguments.get("peer-address"),
        )
        nh_path = build_path(
            "/network-instance[name={name}]/route-table/next-hop[index=*]",
            name=arguments.get("network-instance"),
        )

        try:
            self._local_as_path_data = state.server_data_store.get_data(
                local_as_path, recursive=False
            )
        except ServerError:
            self._local_as_path_data = None

        try:
            self._peer_path_data = state.server_data_store.get_data(
                peer_path, recursive=True
            )
        except ServerError:
            self._peer_path_data = None

        try:
            self._nh_path_data = state.server_data_store.get_data(
                nh_path, recursive=True
            )
        except ServerError:
    ...
    def _populate_schema(self, arguments):
        ...
        for netinst in self._local_as_path_data.network_instance.items():
            ...
            if "*" != arguments.get("peer-as"):
                for peeraddr in (
                    self._peer_path_data.network_instance.get(netinst.name)
                    .protocols.get()
                    .bgp.get()
                    .neighbor.items()
                ):
                ...
                for peeraddr in (
                    ...
                    peers.append(peeraddr.peer_address)
            for nh_index in (
                self._nh_path_data.network_instance.get(netinst.name)
                .route_table.get()
                .next_hop.items()
            ):
                if nh_index.ip_address in peers:
                    netinst_node.neighbor.get(
                        nh_index.ip_address
                    ).interface = nh_index.subinterface
    ...
    def _set_formatters(self, schema):
        ...
        schema.set_formatter(
            ...
            Indent(ColumnFormatter(ancestor_keys=False), indentation=2),
        )
