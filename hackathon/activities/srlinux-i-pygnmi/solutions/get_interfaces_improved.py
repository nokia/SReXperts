...
def get_evi(gc):
    return gc.get(
        path=[
            "/network-instance[name={}]/protocols/bgp-evpn/bgp-instance[id=1]/evi".format(
                args.mac_vrf
            )
        ]
    )["notification"][0]["update"][0]["val"]


def get_vni(gc):
    vxlan_if = gc.get(
        path=["/network-instance[name={}]/vxlan-interface".format(args.mac_vrf)]
    )["notification"][0]["update"][0]["val"]["vxlan-interface"][0]
    (name, subintf) = vxlan_if["name"].split(".")
    return gc.get(
        path=[
            "/tunnel-interface[name={}]/vxlan-interface[index={}]/ingress/vni".format(
                name, subintf
            )
        ]
    )["notification"][0]["update"][0]["val"]
...
def get_connected_ipvrf(gc):
    ...
    ipvrfs = gc.get(path=["/network-instance[name=*]/type"])["notification"][0][
        "update"
    ][0]["val"]["srl_nokia-network-instance:network-instance"]
    for ipvrf in ipvrfs:
        if ipvrf["type"].endswith("ip-vrf"):
            ipvrf_interfaces = gc.get(
                path=["/network-instance[name={}]/interface".format(ipvrf["name"])]
            )["notification"][0]["update"][0]["val"]["interface"]
...
def get_irb_ip(gc):
    ...
    ip = gc.get(
        path=[
            "/interface[name={}]/subinterface[index={}]/ipv4/address".format(
                irb_name, irb_subint
            )
        ]
    )["notification"][0]["update"][0]["val"]["address"][0]
    return ip["ip-prefix"]
...
def get_interfaces(gc):
    ...
    for intf in interfaces:
        if not intf["name"].startswith("irb"):
            ...
            vlan = gc.get(
                path=[
                    "/interface[name={}]/subinterface[index={}]/vlan/encap/single-tagged".format(
                        intf_name, intf_subint
                    )
                ]
            )["notification"][0]["update"][0]["val"]
...
with gNMIclient(
    ...
    print(
        "This interface is connected to IP-VRF '{}' with IP '{}'\n".format(
            get_connected_ipvrf(gc)["name"], get_irb_ip(gc)
        )
    )
    ...
    print(
        tabulate(interfaces, headers=["Interface", "Sub-interface", "VLAN", "Status"])
    )
    print()
